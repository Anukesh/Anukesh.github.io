...
`;const p=document.querySelector("#name"),f=document.querySelector("#city"),h=document.querySelector("#state"),c=document.querySelector("#phone"),d=document.querySelector("#email"),r=document.querySelector("#userForm"),l=document.querySelector("#submit");c.addEventListener("input",g);d.addEventListener("input",y);r.addEventListener("input",_);r.addEventListener("submit",v);document.querySelectorAll(".form__input").forEach(t=>{t.addEventListener("blur",()=>{t.classList.add("form__input--touched")})});function g(t){let e=t.target.value.replace(/\D/g,"");e.length>10&&(e=e.slice(0,10));let n="";e.length>0&&(n+="("+e.slice(0,3)),e.length>=4&&(n+=") "+e.slice(3,6)),e.length>=7&&(n+="-"+e.slice(6,10)),t.target.value=n,/^\(\d{3}\) \d{3}-\d{4}$/.test(t.target.value)?t.target.setCustomValidity(""):t.target.setCustomValidity("Invalid phone format")}function y(t){/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t.target.value)?t.target.setCustomValidity(""):t.target.setCustomValidity("Invalid phone format")}function _(){console.log(r.checkValidity()),l.disabled=!r.checkValidity()}async function v(t){t.preventDefault(),l.disabled=!0,l.innerHTML="Submitted!";const e={name:p.value,city:f.value,state:h.value,phone:c.value,email:d.value};try{const n=await fetch("https://formsws-hilstaging-com-0adj9wt8gzyq.runscope.net/solar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(e)});console.log("Form submitted successfully:")}catch{console.error("Submission failed:")}}
